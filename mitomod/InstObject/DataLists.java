...
public class DataLists {
	...
	public static Map<World, BB_DataWorld> worldDataMap = new HashMap<World, BB_DataWorld>();
	...
	public static BB_DataChunk getChunkData(Chunk chunk) {
		return DataLists.getChunkData(chunk.worldObj, chunk.xPosition, chunk.zPosition);
	}
	...
	public static BB_DataChunk getChunkData(World world, int i, int j) {
		...
		if (ret == null) {
			ret = DataLists.newDataChunk(world, i, j);
		}
		return ret;
	}
	...
	public static boolean isChunkExist(World world, int i, int j) {
		return DataLists.getWorldData(world).coordToDataMapping.containsItem(ChunkCoordIntPair.chunkXZ2Int(i, j));
	}
}
