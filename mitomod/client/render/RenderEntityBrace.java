...
public class RenderEntityBrace extends Render {
	...
	private EntityBrace ent;

	public static final double WireLength = 10;

	public void doRender(Entity entity, double x, double y, double z, float par8, float par9) {

		if (entity instanceof EntityBrace && ((EntityBrace)entity).getData() != null) {
			this.ent = (EntityBrace) entity;
			Tessellator tessellator = Tessellator.instance;
			tessellator.setColorOpaque_F(1.0F, 1.0F, 1.0F);
			this.bindTexture(this.tex[ent.getTexture()]);
			renderBrace(ent, ent.getSet(), ent.getEnd(), (double) ent.getSize() * 0.05, x, y, z);
		} else if (entity instanceof EntityFake) {
			EntityFake fake = (EntityFake)entity;
			if(fake.size == 0){
				GL11.glPushMatrix();
	...
	public void renderBrace(EntityBrace ent, Vec3 set, Vec3 end, double size, double x, double y, double z) {
		...
		GL11.glTranslatef((float) ent.getData().rand.xCoord, (float) ent.getData().rand.yCoord, (float) ent.getData().rand.zCoord);
		Tessellator tessellator = Tessellator.instance;
		...
		if (ent.isBent) {
			...
			if (ent.getType() == 1) {
				RenderCore.renderSimpleBezier(20, ent.getSet(), ent.getSetCP(), ent.getEndCP(), ent.getEnd(), ent.getSize(), alpha);
			} else if (ent.getType() == 3) {
				RenderCore.renderSimpleBezier(20, ent.getSet(), ent.getSetCP(), ent.getEndCP(), ent.getEnd(), ent.getSize(), alpha);
			} else if (ent.getType() == 4) {
				RenderCore.renderSimpleBezier(20, ent.getSet(), ent.getSetCP(), ent.getEndCP(), ent.getEnd(), ent.getSize(), alpha);
			} else if (ent.getType() == 5 || ent.getType() == 7) {
				RenderCore.renderQuadBezier(20, ent.getSet(), ent.getSetCP(), ent.getEndCP(), ent.getEnd(), size, size / 5, alpha);
			} else if (ent.getType() == 6 || ent.getType() == 8) {
				RenderCore.renderQuadBezier(20, ent.getSet(), ent.getSetCP(), ent.getEndCP(), ent.getEnd(), size / 5, size, alpha);
			} else {
				RenderCore.renderQuadBezier(20, ent.getSet(), ent.getSetCP(), ent.getEndCP(), ent.getEnd(), size, size, alpha);
			}
		} else {
		...
		if (ent.getType() != 2 && ent.getType() != 3 && ent.getType() != 4 && ent.getType() != 7 && ent.getType() != 8) {
			GL11.glPushMatrix();

			GL11.glTranslated(-ent.posX, -ent.posY, -ent.posZ);
			GL11.glTranslatef((float) x, (float) y, (float) z);
			GL11.glTranslatef((float) ent.getSet().xCoord, (float) ent.getSet().yCoord, (float) ent.getSet().zCoord);
			...
			GL11.glTranslated(-ent.posX, -ent.posY, -ent.posZ);
			GL11.glTranslatef((float) x, (float) y, (float) z);
			GL11.glTranslatef((float) ent.getEnd().xCoord, (float) ent.getEnd().yCoord, (float) ent.getEnd().zCoord);
			renderBox(size * d, size * d, size * d, alpha);
			GL11.glPopMatrix();
		}
